...
for (pop in pops){
    ...
    if (pop == "CARL"){
        ...
        selected_panels <- c("INGI.shapeit","1000Gph1.shapeit","1000GP_Phase3.shapeit","INGI_1000GPh3.shapeit")
    }
    ...
    for(panel in selected_panels){
        ...
        for (chr in 1:1){
            ...
            current_pop_current_panel_current_chr_info <-  read.table(current_pop_current_panel_current_chr_info_name,header=T,sep=" ",stringsAsFactors=F, comment.char="",colClasses=c("integer","character","integer",rep("numeric",2),"integer","NULL",rep("numeric",2),rep("character",2),rep("numeric",2)))
    ...
    # cdata_nomono <- cdata
    cdata_nomono$BIN2 <- 1
    cdata_nomono[which(cdata_nomono$BIN == 0.01),]$BIN2 <- 2
    cdata_nomono[which(cdata_nomono$BIN == 0.02),]$BIN2 <- 3
    cdata_nomono[which(cdata_nomono$BIN == 0.05),]$BIN2 <- 4
    cdata_nomono[which(cdata_nomono$BIN == 0.10),]$BIN2 <- 5
    cdata_nomono[which(cdata_nomono$BIN == 0.15),]$BIN2 <- 6
    cdata_nomono[which(cdata_nomono$BIN == 0.20),]$BIN2 <- 7
    cdata_nomono[which(cdata_nomono$BIN == 0.25),]$BIN2 <- 8
    cdata_nomono[which(cdata_nomono$BIN == 0.30),]$BIN2 <- 9
    cdata_nomono[which(cdata_nomono$BIN == 0.40),]$BIN2 <- 10
    cdata_nomono[which(cdata_nomono$BIN == 0.50),]$BIN2 <- 11
    ...
    pl <- pl + aes(x = BIN2, y = mean, colour=PANNELLO,group=PANNELLO)
...
for (pop in pops){
    ...
    pl <- pl + theme(axis.title= element_text(size=rel(1.2)))
    pl <- pl + theme(legend.text= element_text(size = rel(1.2)), legend.title = element_text(size = rel(1.2)),legend.position = c(0.8, 0.2))
    ggsave(filename=paste(getwd(),"/complete_19102015_r2_",pop,"_info.jpeg",sep=""),width=12, height=7,dpi=300,plot=pl)
