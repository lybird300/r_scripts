...
# from INFO SCORE stratifying by panel and frequency
...
for (pop in pops){
    ...
    for(panel in all_panels){
        ...
        for (chr in 1:22){
            ...
            current_pop_current_panel_current_chr_info_name <- paste(pop_folder,"/",chr,"/chr",chr,".gen_info_partial_INFOBIN_t2.gz",sep="")
            ...
            current_pop_current_panel_current_chr_info <-  read.table(current_pop_current_panel_current_chr_info_name,header=T,sep=" ",stringsAsFactors=F, comment.char="",colClasses=c("integer","character","integer",rep("numeric",2),"character","NULL","NULL","NULL",rep("character",2),rep("numeric",3)))
    ...
    for (pan_set in all_set){
        ...
        current_pop_all_panels_all_chr_nomono <- current_pop_all_panels_all_chr[which(current_pop_all_panels_all_chr$MAF != 0),]
        ...
        ###################################################################################
        cdata <- ddply(current_pop_all_panels_all_chr_nomono, c("INFO_BIN","PANEL"), summarise,
        N    = length(INFO),
        mean = mean(INFO),
        median = median(INFO),
        sd   = sd(INFO),
        ...
        # min_freq <- maf_bins[2]
        out_folder <- paste(base_folder,"/PLOTS/",current_date,"_INFO_BINS","/FIG/",pan_set,"/",pop,"_",mode,sep="")
        tab_folder <- paste(base_folder,"/PLOTS/",current_date,"_INFO_BINS","/TAB/",pan_set,"/",pop,"_",mode,sep="")
        ...
        pl <- pl + facet_wrap( ~ INFO_BIN,scales = "free_y")
        # pl <- pl + scale_color_discrete(name=paste(pop," on:"))
