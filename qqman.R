...
manhattan = function(dataframe, colors=c("gray10", "gray50"), ymax="max", cex.x.axis=1, limitchromosomes=1:23, suggestiveline=-log10(1e-5), genomewideline=-log10(5e-8), annotate=NULL, ...) {
...
qq = function(pvector,unfiltered=NULL, ...) {
    if (!is.numeric(pvector)) stop("D'oh! P value vector is not numeric.")
	pvector <- pvector[!is.na(pvector) & pvector<1 & pvector>0]
    o = -log10(sort(pvector,decreasing=F))
	#if we are downsampling our result for plotting, we need to remove 
	if (!is.null(unfiltered)) {	
		e_all <- -log10(1:unfiltered/unfiltered)
		start_index <- (unfiltered - length(o)) + 1 
		e <- e_all[start_index:length(e_all)]

    }else{    	
		e = -log10( 1:length(o)/length(o) )
    }
	# e = -log10(ppoints(length(pvector)))
	plot(e,o,pch=19,cex=1, xlab=expression(Expected~~-log[10](italic(p))), ylab=expression(Observed~~-log[10](italic(p))), xlim=c(0,max(e)), ylim=c(1,max(o)+1),yaxs="i", ...)
	# abline(e,e,col="red")
	abline(1,1,col="red")
}
